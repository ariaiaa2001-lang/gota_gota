    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form
          onSubmit={(e) => {
            ...
            startTransition(async () => {
              await createPaymentAction(loanId, clientId, formData)
              setOpen(false)
            })
          }}
        ...
        >
          ...
          <Button type="submit" disabled={isPending} className="w-full bg-emerald-500 hover:bg-emerald-600 h-12 font-bold text-lg">
...
export function EditPaymentModal({ payment, clientId, updatePaymentAction }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Corregir Abono</DialogTitle>
        </DialogHeader>
        ...
        <form
          onSubmit={(e) => {
            ...
            startTransition(async () => {
              await updatePaymentAction(payment.id, clientId, formData)
              setOpen(false)
            })
          }}
          className="space-y-4 pt-4"
        >
          <div className="space-y-1">
            ...
            <Input name="amount" type="number" defaultValue={payment.amount} required className="text-xl font-bold" />
          </div>

          <div className="space-y-1">
            <Label>Fecha Real</Label>
            <Input name="date" type="datetime-local" defaultValue={new Date(payment.created_at).toISOString().slice(0, 16)} required />
          </div>
        ...
        <DialogFooter>
          <form
            onSubmit={(e) => {
              ...
              startTransition(async () => {
                await deletePaymentAction(payment.id, clientId)
                setOpen(false)
              })
            }}
            className="w-full"
          >
            <Button type="submit" disabled={isPending} variant="destructive" className="w-full bg-rose-600 font-bold">
              {isPending ? 'Eliminando...' : 'SÍ, ELIMINAR AHORA'}
            </Button>
          </form>
        </DialogFooter>
      </DialogContent>
