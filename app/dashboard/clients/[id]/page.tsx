...
export default async function ClientDetailsPage({ params }: PageProps) {
  ...
  // 2. Traemos los datos del cliente (Simple)
  ...
  // 3. Traemos préstamos y pagos por separado para asegurar que carguen
  ...
  // Calculamos deuda total basándonos en la tabla de préstamos que acabamos de traer
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Encabezado (Tu diseño de la imagen) */}
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="text-xs font-bold uppercase text-muted-foreground">Préstamos Activos e Historial</CardTitle>
          </CardHeader>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {loans && loans.length > 0 ? loans.map((loan: any) => (
                <TableRow key={loan.id}>
                  <TableCell className="font-medium text-sm">{formatCurrency(loan.total_amount)}</TableCell>
                  <TableCell className="font-bold text-red-600 text-sm">{formatCurrency(loan.remaining_balance)}</TableCell>
                  <TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={3} className="text-center py-4 text-slate-400 text-xs italic">Sin préstamos registrados</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
      ...
      <Card>
        <CardHeader className="bg-slate-50/50 border-b">
          <CardTitle className="text-xs font-bold uppercase text-muted-foreground flex items-center gap-2">
            <History className="h-4 w-4" /> Registro de Cobros Recibidos
          </CardTitle>
        </CardHeader>
        <Table>
          <TableHeader>
